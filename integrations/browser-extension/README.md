...
5. Select the settings control in the side-panel header, then choose **Connect this browser** to sign in on the Maillume account page, review the browser connection, and approve it with 2FA. The extension receives the new credential once, returns to email review, and completes setup automatically. No API key needs to be copied.
...
8. Press **Use current message** after opening another email to clear the previous assessment and restore the review fields. The capture instructions can be hidden when more room is useful.
...
The panel accepts the versioned `analysis-v6` through `analysis-v12` classification and score-factor contracts, sends its extension ID/version and accepted analysis versions, verifies that factor contributions sum to the risk index, requires valid evidence coverage from `analysis-v9` onward, and rejects unsafe detected-link schemes. Version 0.4.0 added stable per-browser rotation, rolling inactivity expiry, and simplified connection settings. Version 0.4.1 adds bounded analysis-response handling and the corresponding privacy disclosure. Version 0.4.2 uses the full panel width for the capture and review workflow instead of a separate numbered gutter, displays the packaged Maillume mark in its header, and moves connection state and controls into a dedicated Settings view. Installed 0.3.9 clients remain compatible with analysis and pairing while the Store update rolls out.

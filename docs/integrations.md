...
- Extension `0.3.9` negotiates compatibility through `/api/v1/capabilities` and sends its official extension ID, version, and accepted analysis versions with hosted requests. The server returns `426` for a known incompatible client.
- **Connect this browser** opens a localized account approval page instead of asking the user to copy a secret. Approval requires a signed-in AAL2 session and creates a dedicated 90-day key named for the browser platform. The account page remains the place to inspect last use, rotate, or revoke it.
- The ten-minute pairing record stores a hashed device verifier, bounded requested key metadata, state, owner after approval, and timestamps. It never stores account credentials, sessions, API-key plaintext, scan content, or results, and a cleanup job purges expired rows.
- The endpoint is stored locally. The API key and expiry can be kept in trusted extension-local storage when the user explicitly chooses to remember them across restarts and updates, or in Chrome session storage otherwise. Captured text crosses to the tab-specific panel through a one-time in-memory handoff and message content and results are never written to extension storage.
...
- Manual key entry remains available for unpacked development builds, self-hosted troubleshooting, and older deployments. The automatic flow is restricted to the official Store extension ID.

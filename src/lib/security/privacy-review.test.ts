...
function main() {
  ...
  assert.match(nextConfigContent, /Strict-Transport-Security/);
  assert.match(nextConfigContent, /max-age=63072000; includeSubDomains; preload/);
  assert.match(cspConfig, /frame-ancestors 'none'/);
  assert.match(cspConfig, /default-src \$\{SELF_SOURCE\}/);
  assert.match(cspConfig, /object-src 'none'/);
  assert.match(cspConfig, /'nonce-\$\{nonce\}'/);
  assert.match(cspConfig, /'strict-dynamic'/);
  assert.doesNotMatch(cspConfig, /script-src[^\n]*unsafe-inline/);
  assert.match(middlewareContent, /headers\.set\("x-nonce", nonce\)/);
  assert.match(middlewareContent, /headers\.set\("Content-Security-Policy", contentSecurityPolicy\)/);
  assert.match(middlewareContent, /response\.headers\.set\("Content-Security-Policy", contentSecurityPolicy\)/);
  assert.match(middlewareContent, /supabaseUrl: process\.env\.NEXT_PUBLIC_SUPABASE_URL/);
  ...
  assert.match(cspConfig, /worker-src \$\{SELF_SOURCE\} blob:/);
  assert.match(cspConfig, /upgrade-insecure-requests/);
  assert.doesNotMatch(cspConfig, /office\.com|outlook\.com/);
  ...
  assert.match(runtimeAuditWorkflow, /payload\.analysis_version !== "analysis-v11"/);

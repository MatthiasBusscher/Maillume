...
test("allows only the normalized configured Supabase HTTPS and WebSocket origins", () => {
  assert.deepEqual(
    getCspConnectSources(" https://project.supabase.co/ "),
...
test("supports explicit local HTTP configuration", () => {
  assert.deepEqual(
    getCspConnectSources("http://127.0.0.1:54321/"),
    ["'self'", "http://127.0.0.1:54321", "ws://127.0.0.1:54321"],
  );
});

test("supports IPv6 loopback HTTP configuration", () => {
  assert.deepEqual(
    getCspConnectSources("http://[::1]:54321/"),
    ["'self'", "http://[::1]:54321", "ws://[::1]:54321"],
  );
});
...
test("rejects malformed, credentialed, and unsupported URLs", () => {
  ...
  assert.deepEqual(getCspConnectSources("http://project.supabase.co"), ["'self'"]);
  assert.deepEqual(getCspConnectSources("http://192.0.2.10:54321"), ["'self'"]);
  assert.deepEqual(getCspConnectSources("https://project.supabase.co/auth/v1"), ["'self'"]);
  assert.deepEqual(getCspConnectSources("https://project.supabase.co?destination=internal"), ["'self'"]);

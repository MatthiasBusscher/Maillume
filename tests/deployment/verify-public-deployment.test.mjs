...
test("waits until the public route serves the approved revision", async (t) => {
  ...
  const server = await createServer((request, response) => {
    ...
    if (request.url === "/api/analyze") {
      response.setHeader("Cache-Control", "no-store");
      response.setHeader("Content-Type", "application/json");
      response.end(JSON.stringify({ analysis_version: "analysis-v8", result: { risk_score: 0 } }));
      return;
    }
    response.end("ok");
  });
...
test("rejects Cloudflare email obfuscation on the marketing page", async (t) => {
  const server = await createServer((request, response) => {
    ...
    if (request.url?.startsWith("/api/health")) {
      response.setHeader("Cache-Control", "no-store");
      response.setHeader("Content-Type", "application/json");
      response.end(JSON.stringify({ status: "ok", revision, analysis_version: "analysis-v8" }));
      return;
    }
    if (request.url === "/api/analyze") {
      response.setHeader("Cache-Control", "no-store");
